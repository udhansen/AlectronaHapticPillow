    ...
    if (central) {
        ...
        while (central.connected()) {
            if (ledCharacteristic.written()) {
                ...
                drawWifiSymbol(SCREEN_WIDTH - 16, 8); // Show Wifi symbol
                }
                else if (ledState == '0') {
...
void drawWifiSymbol(int x, int y) {
    // Draw outer arcs for WiFi symbol
    display.drawCircle(x, y, 7, SSD1306_WHITE); // Outer arc
    display.drawCircle(x, y, 4, SSD1306_WHITE);  // Middle arc
    display.drawCircle(x, y, 2, SSD1306_WHITE);  // Inner arc
